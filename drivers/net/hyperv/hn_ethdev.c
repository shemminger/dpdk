...
static struct rte_vmbus_driver rte_hyperv_pmd = {
	.id_table = hn_net_ids,
	.probe = eth_hn_probe,
	.remove = eth_hn_remove,
};

RTE_PMD_REGISTER_VMBUS(net_hyperv, rte_hyperv_pmd);
RTE_PMD_REGISTER_KMOD_DEP(net_hyperv, "* uio_hv_generic");

RTE_INIT(hn_init_log);
static void
hn_init_log(void)
{
	hn_logtype_init = rte_log_register("pmd.net.hyperv.init");
	if (hn_logtype_init >= 0)
		rte_log_set_level(hn_logtype_init, RTE_LOG_NOTICE);
	hn_logtype_driver = rte_log_register("pmd.net.hyperv.driver");
